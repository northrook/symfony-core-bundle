...
final class AutoConfigure
{
    ...
    private function createConfigFile( string $name, string $config ) : void {
        ...
        if ( !str_starts_with( '<?php', $config ) ) {
            Output::error(
                'AutoConfigure could not create ' . $name . ' file, it does not appear to be a valid PHP file.',
            );
            return;
        }
        ...
        try {
            ( new Filesystem() )->dumpFile( $path, $config );
            Output::OK( "AutoConfigure: Generated config/$name." );
        }
        catch ( IOException $e ) {
            $message = "AutoConfigure: Could not generate config/$name. {$e->getMessage()}";
            Log::Error( message : $message, context : [ 'exception' => $e ] );
            Output::error( $message );
        }
    }
